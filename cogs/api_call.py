...
class Api(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.meme_cache = MemeTemplateCache()

    @commands.slash_command(name="duck", description="Random duck image")
    async def duck_fact(self, ctx: discord.ApplicationContext):
    ...
    @commands.slash_command(name="meme", description="Browse meme templates and select one")
    async def meme(self, ctx):
        templates = self.meme_cache.fetch_templates()
        if not templates:
